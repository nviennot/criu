...
# Instead of passing the location of the image streamer as an argument (e.g.,
# --criu-image-streamer-bin), the caller can modify PATH to point to the right
# location. We add a default location as well.
IMG_STREAMER_PATH = os.path.realpath(os.path.join(SCRIPT_DIR, "../../criu-image-streamer"))
os.environ['PATH'] = "{}:{}".format(os.environ['PATH'], IMG_STREAMER_PATH)
...
class criu:
    ...
    def dump(self, action, opts=[]):
        ...
        if self.__stream:
            ...
            a_opts += ["--stream"]
            # TODO test for additional messages such as the checkpoint-start
            # message, which occur right after CRIU starts dumping
